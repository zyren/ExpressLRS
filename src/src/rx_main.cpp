...
static void HandleUARTout()
{
    if (firmwareOptions.is_airport)
    {
        ...
        if (OtaNonce % 64 == 1 || percentageFull > 50)
        {
            ...
            // RC channels code for later:

            // uint8_t buf[MAVLINK_MAX_PACKET_LEN];
            // mavlink_message_t msg;
            // mavlink_msg_rc_channels_override_pack(255, 0, &msg, 1, 1, CRSF_to_US(crsf.ChannelData[0]), CRSF_to_US(crsf.ChannelData[1]), CRSF_to_US(crsf.ChannelData[2]), CRSF_to_US(crsf.ChannelData[3]),1000,1100,1200,1300,1400,1500,1600,1700,1800,1900,2000,1000,1500,2000);
            // uint16_t len = mavlink_msg_to_send_buffer(buf, &msg);
            // Serial.write(buf, len);
        }
        ...
        while (apOutputBuffer.size())
        {
            ...
            if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status))
            {
                ...
                // For all other mavlink messages, foward them on to the FC
