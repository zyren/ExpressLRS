...
bool LR1121Driver::CheckVersion(const SX12XX_Radio_Number_t radioNumber)
{
    ...
    if (!SPIFFS.exists("/lr1121.txt") && (version.type != LR1121_FIRMWARE_TYPE || version.version != LR1121_FIRMWARE_VERSION))
    {
        DBGLN("Upgrading radio #%d", radioNumber);
        // do upgrade
        if (BeginUpdate(radioNumber, sizeof(flash)) != 0) return false;
        uint8_t dest[256];
        for (int pos = 0 ; pos < sizeof(flash) / 4 ; pos += 64)
        {
            uint32_t size = 256;
            if (pos + 63 > sizeof(flash) / 4) size = sizeof(flash) % 256;
            memcpy(dest, flash + pos, size);
        ...
        if (version.type != LR1121_FIRMWARE_TYPE && version.version != LR1121_FIRMWARE_VERSION)
        {
